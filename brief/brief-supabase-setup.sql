...
-- Run in Supabase Studio -> SQL Editor -> New query -> paste ALL -> Run.
...
create table if not exists public.brief_articles (
  ...
  -- Scratch fields for the future automated research/draft system.
  ai_score numeric,
  ai_sources jsonb,
  ai_model text,
  ai_notes text
...
-- A signed-in editor (auth.role() = 'authenticated') is unaffected.
-- ---------------------------------------------------------------------
...
-- Helper: is the current request the signed-in editor?
...
-- =====================================================================
-- Seed content. Dollar-quoted strings ($md$...$md$) so no apostrophe
-- escaping is needed. Per-row ON CONFLICT (slug) DO NOTHING makes this
-- safe to re-run.
-- =====================================================================
insert into public.brief_articles
  (slug, status, source, content_type, category, headline, standfirst, body, author,
   brand, nova_take, source_name, tags, homepage_slot, reading_minutes, published_at,
   seo_title, meta_description)
values
(
  'hovis-4m-brand-campaign', 'published', 'human', 'Campaign Breakdown', 'Campaigns',
  $md$Hovis just spent £4m reminding Britain why it knows its brand$md$,
  $md$The bread maker's biggest push in a decade leans hard on nostalgia and distinctive assets, and it is a masterclass in spending media money on what a brand already owns.$md$,
  $md$Hovis has brought back its 1973 "Boy on the Bike" cue: the cobbled hill, the brass-band score, the Yorkshire accent, across TV, cinema, out-of-home and retail media, with almost no new brand-building beyond it.
...
The risk is not the idea. It is whether £4m is enough weight behind it to move a share point in a huge, low-interest category.$md$,
  'Nova Brief', 'Hovis',
  $md$Reassuringly boring, in the best way. The smartest media plans often spend money on what the brand already owns rather than chasing something new.$md$,
  'Nova Brief analysis',
  array['Hovis','distinctive assets','brand building','FMCG'],
  'campaign_of_week', 4, now() - interval '1 hour',
  $md$Hovis's £4m brand campaign, broken down$md$,
  $md$Why Hovis put £4m behind a near-total return to its 1973 brand cues, and what it says about spending on distinctive assets.$md$
),
(
  'asda-brand-refresh-2026', 'published', 'human', 'News', 'Branding',
  $md$Asda unveils a full brand refresh as it fights to win back shoppers$md$,
  $md$New logo lock-up, a warmer palette and a "price" platform front and centre. The supermarket says it is the biggest identity change in 15 years.$md$,
  $md$Asda has rolled out a refreshed identity across stores, packaging and advertising, its most significant visual change since 2011.
...
A refresh will not fix a positioning problem on its own. Asda's challenge is share loss to both the discounters below it and a resurgent Sainsbury's and Tesco above it. A clearer, more confident identity helps, but only if the price promise behind it is real.$md$,
  'Nova Brief', 'Asda', null,
  'Company announcement',
  array['Asda','retail','rebrand','supermarkets'],
  'top_story', 3, now() - interval '3 hours',
  null,
  $md$Asda has launched its biggest brand refresh in 15 years, with price pushed to the centre of the identity.$md$
),
(
  'instagram-trial-reverse-chronological', 'published', 'human', 'News', 'Social',
  $md$Instagram tests a reverse-chronological main feed for all users$md$,
  $md$An opt-in toggle is rolling out in the UK and US. Adam Mosseri says it is "a response to what people keep asking for", not a change to the default.$md$,
  $md$Instagram has begun testing an opt-in setting that shows the main feed in reverse-chronological order, rolling out first in the UK and US.
...
Not much, yet. Reach is still overwhelmingly ranked. But it is a signal worth watching: every chronological experiment so far has fed back into the way the ranked feed weights recency.$md$,
  'Nova Brief', 'Instagram', null,
  'Adam Mosseri',
  array['Instagram','Meta','social platforms','feed ranking'],
  'quick_read', 2, now() - interval '1 day',
  null,
  $md$Instagram is testing an opt-in chronological main feed in the UK and US. The default ranking is unchanged.$md$
),
(
  'uncommon-wins-british-airways', 'published', 'human', 'News', 'Agencies',
  $md$Uncommon wins British Airways' global creative account$md$,
  $md$The airline consolidates with a single shop after a four-way pitch. First work is expected in early 2027, built around a refreshed platform.$md$,
  $md$British Airways has appointed Uncommon Creative Studio as its global creative agency following a four-way competitive pitch.
...
Uncommon has a strong record turning heritage brands into modern, emotionally-led campaigns. The question is whether an airline's operational realities (delays, pricing, loyalty changes) leave enough room for the kind of work Uncommon is known for.$md$,
  'Nova Brief', 'British Airways', null,
  'Agency statement',
  array['British Airways','Uncommon','account moves','airlines'],
  'quick_read', 3, now() - interval '2 days',
  null,
  $md$British Airways has appointed Uncommon as its global creative agency after a four-way pitch.$md$
),
(
  'what-marketers-need-to-know-ai-search', 'published', 'human', 'Analysis', 'AI',
  $md$What marketers actually need to know about AI search this quarter$md$,
  $md$Cutting through the noise: where AI-generated answers are actually changing traffic, and the three things worth doing now.$md$,
  $md$AI answers are now a standard part of the search results page for a large share of informational queries. The effect on brand traffic is real but uneven.

## Where it bites

Top-of-funnel explainer content is losing clicks to summarised answers. Transactional and branded queries are largely unaffected so far.
...
> The brands doing well in AI answers are the ones that were already the clearest, best-structured sources on their topic.$md$,
  'Nova Brief', null, null,
  'Nova Brief analysis',
  array['AI search','SEO','content strategy','generative engines'],
  'quick_read', 6, now() - interval '2 days',
  null,
  $md$Where AI-generated search answers are actually changing brand traffic, and the three things worth doing about it now.$md$
),
(
  'retail-media-second-wave', 'published', 'human', 'Analysis', 'Digital',
  $md$Retail media's second wave is here, and it is coming for brand budgets$md$,
  $md$For five years retail media was funded out of shopper marketing. Nova Brief spoke to eight brand and agency leaders about why that is changing.$md$,
  $md$Retail media networks have spent five years growing on trade and shopper-marketing money. The next phase is different: networks want brand budgets, and they are building the ad formats to justify it.
...
> Every leader we spoke to is spending more on retail media next year. Not one was fully confident in the way it is measured.

The brands getting value are treating retail media as a testable channel with its own hypotheses, not as a tax on doing business with a retailer.$md$,
  'Nova Brief', null,
  $md$The winners here will be the brands that demand the same measurement rigour from retail media that they would expect from any other channel.$md$,
  'Nova Brief reporting',
  array['retail media','commerce','measurement','media planning'],
  'story_of_week', 8, now() - interval '4 days',
  null,
  $md$Retail media is moving from trade spend to brand budgets. Eight leaders on what that changes, and where measurement still falls down.$md$
),
(
  'nova-take-ai-content-missing-the-point', 'published', 'human', 'Nova Take', 'AI',
  $md$Why everyone chasing AI content is missing the point$md$,
  $md$The bottleneck was never speed of publishing.$md$,
  $md$The promise of AI content tools is volume: more posts, more pages, more variants, faster. But for most brands, volume was never the constraint.
...
> If your content had a reason to exist before AI, these tools will help you make more of it. If it did not, they will help you make that problem bigger.

Use the time AI gives back on the part that was always hard: deciding what actually deserves to be published.$md$,
  'Nova Brief', null, null,
  null,
  array['AI','content strategy','opinion'],
  'focus_ai', 3, now() - interval '5 days',
  null,
  $md$AI content tools solve for volume. For most brands, volume was never the problem.$md$
),
(
  'lidl-christmas-2026-teaser', 'published', 'human', 'News', 'Campaigns',
  $md$Lidl kicks off the Christmas ad race early with a raccoon and a wink at John Lewis$md$,
  $md$A 15-second teaser dropped this week, a full three months out. The discounter is betting that being first is worth more than being polished.$md$,
  $md$Lidl has released a short teaser for its Christmas campaign in early September, well ahead of the traditional November window.
...
Going early is a media strategy as much as a creative one. Impressions are cheaper now, share of voice is easier to win, and the campaign gets more weeks on air for the same idea.$md$,
  'Nova Brief', 'Lidl', null,
  'Brand social channels',
  array['Lidl','Christmas advertising','retail','seasonal'],
  'none', 2, now() - interval '5 days',
  null,
  $md$Lidl has released a Christmas campaign teaser in early September, betting that being first beats being polished.$md$
)
...
values
  ...
  ('73%', $md$of marketers say they have cut content volume this year and shifted budget to fewer, bigger ideas.$md$, 'Trends', 'Nova Brief reader survey, Aug 2026', null, 2, 'published'),
  ('1 in 3', $md$UK product searches now start on a retail media network rather than Google or a brand site.$md$, 'Digital', 'Industry estimate, 2026', null, 3, 'published');
...
values (
  ...
  $md$[
    ...
    {"headline":"Retail media second wave targets brand budgets","summary":"Networks are building full-funnel formats and the measurement gap is showing.","category":"Digital","link":"/retail-media-second-wave"},
...
values
  ('hovis', 'Hovis', $md$One of Britain's best-known bread brands, and a recurring case study in distinctive-asset marketing.$md$),
  ('asda', 'Asda', $md$The UK's third-largest supermarket, working through a brand and positioning reset.$md$),
  ('british-airways', 'British Airways', $md$The UK flag carrier and its long search for a consistent modern brand platform.$md$)
on conflict (slug) do nothing;
